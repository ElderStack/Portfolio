 ...
 * - strawhat_cs.c (StrawHat VM)
 * - Copyright: Prof. Kevin Andrea, George Mason University.  All Rights Reserved
 * - Date: Jan 2025
 *
 *   Main support for the Context Switch system in the StrawHat VM.
...
static Scheduler_process_s *on_cpu = NULL;
static Scheduler_s *schedule = NULL;
...
static long sleep_usec_time = SLEEP_USEC;
static long between_usec_time = BETWEEN_USEC;
...
void initialize_cs_system() {
  ...
  schedule = sched.scheduler_initialize();
  if(schedule == NULL) {
    ABORT_ERROR("Error reported by the Scheduler initialize function.\n");
  }
}
...
void cs_cleanup() {
  ...
  PRINT_STATUS("... Deallocating Scheduler with the Scheduler cleanup(schedule)\n");
  sched.scheduler_cleanup(schedule);
...
// .. e) Returns the process to the Scheduler (insert)
  while(cs_do_cs == CS_RUN) {
    ...
    PRINT_DEBUG("\n");
    PRINT_DEBUG(".----------------------------------\n");
    PRINT_DEBUG("| Context Switch: Iteration %d\n", iteration++);

    // Call the Scheduler to get the next Process
    on_cpu = sched.scheduler_select(schedule);
    if(on_cpu) {
      PRINT_DEBUG("| Schedule Select Returned PID %d (%s)\n", on_cpu->pid, on_cpu->cmd);
    }
    else {
    ...
    if(on_cpu != NULL) {
      if(process_find(on_cpu->pid) == 0) {
        if(sched.scheduler_exited(schedule, on_cpu, 42) == -1) {
          ABORT_ERROR("Error reported by the Scheduler exited function.\n");
        }
        on_cpu = NULL;
        ...
        if(last_run_cpu != on_cpu->pid) {
          PRINT_STATUS("Switching to run PID: %d (%s)\n", on_cpu->pid, on_cpu->cmd);
        }
        last_run_cpu = on_cpu->pid;
        PRINT_DEBUG("| Running Process with PID: %d (%s)\n", on_cpu->pid, on_cpu->cmd);
        kill(on_cpu->pid, SIGCONT);
        usleep(delay);
        // It's run for the quantum, suspend it and return it to the queue.
        if(on_cpu) {
          kill(on_cpu->pid, SIGTSTP);
          PRINT_DEBUG("| Stopping Process with PID: %d (%s)\n", on_cpu->pid, on_cpu->cmd);
          PRINT_DEBUG("| Enqueueing Process with PID: %d (%s)\n", on_cpu->pid, on_cpu->cmd);
          if(sched.scheduler_enqueue(schedule, on_cpu) == -1) {
            ABORT_ERROR("Error reported by the Scheduler enqueue function.\n");
          }
          on_cpu = NULL;
        }
      }
    ...
    PRINT_DEBUG("| Promoting Processes in Ready Queue Normal\n");
    if(cs_do_cs == CS_RUN && sched.scheduler_promote(schedule) == -1) {
      ABORT_ERROR("Error reported by the Scheduler promote function.\n");
    }
#endif
    PRINT_DEBUG("`----------------------------------\n");
...
void cs_suspend(pid_t pid) {
  ...
  if(sched.scheduler_suspend(schedule, pid) == -1) {
    PRINT_WARNING("The Scheduler suspend function reported no such process.\n");
  }
  if(last_state == CS_RUN) {
    start_cs();
  }
#endif // Feature Suspend Enabled
}
...
void cs_resume(pid_t pid) {
  ...
  if(sched.scheduler_resume(schedule, pid) == -1) {
    PRINT_WARNING("The Scheduler resume function reported no such process.\n");
  }
  if(last_state == CS_RUN) {
    start_cs();
  }
#endif // Scheduler Resume function supported
}
...
void cs_reap(pid_t pid) {
  ...
  int ec = sched.scheduler_reap(schedule, pid);
  if(ec == -1) {
    PRINT_WARNING("[No Such Process to Reap]\n");
  }
  else {
    PRINT_STATUS("Process Reaped.  Exit Code was %d\n", ec);
  }
  if(last_state == CS_RUN) {
    start_cs();
  }
...
void cs_exiting_process(int exit_code) {
  ...
  if(on_cpu) {
    if(sched.scheduler_exited(schedule, on_cpu, exit_code) == -1) {
      ABORT_ERROR("Error reported by the Scheduler exited function.\n");
    }
    PRINT_DEBUG("| Exiting PID %d, with exit code %d with the Scheduler exited\n", on_cpu->pid, exit_code);
    on_cpu = NULL;
  }
  else {
...
void cs_scheduler_process(Process_data_s *proc) {
  ...
  Scheduler_create_data_s *data = strawhat_scheduler_initialize_data(proc);
  Scheduler_process_s *proc_node = sched.scheduler_create(data);
  free(data);
  if(proc_node == NULL) {
    ABORT_ERROR("Error reported by the Scheduler create function.\n");
  }
  // Then Insert it into the Queue
  if(sched.scheduler_enqueue(schedule, proc_node) == -1) {
    ABORT_ERROR("Error reported by the Scheduler enqueue function.\n");
  }
  PRINT_STATUS("Process %s created with PID %d\n", proc->input_orig, proc->pid);
  // Finally, print the schedule out (Debug Mode Only) to see it there.
  print_scheduler_debug(schedule, get_on_cpu());
}
...
void cs_scheduler_terminated(pid_t pid, int exit_code) {
  ...
  // Otherwise, it was terminated while in a Queue; treat as a terminated process.
    ...
    int status = sched.scheduler_killed(schedule, pid, exit_code);
    if(status == -1) {
      ABORT_ERROR("Error reported by the Scheduler killed function.\n");
    }

    PRINT_DEBUG("Terminating PID %d with exit code %d with the Scheduler killed function\n", pid, exit_code);
  }
...
void print_start_cs() {
  PRINT_STATUS("Starting CS System: %ld usec Run, %ld usec Between\n", sleep_usec_time, between_usec_time);
}
...
void print_cs_status() {
  ...
  if(state == CS_RUN) {
    PRINT_STATUS("CS System Running: runtime %ld usec, delaytime %ld usec\n", sleep_usec_time, between_usec_time);
  }
  else {
    PRINT_STATUS("CS System Stopped: runtime %ld usec, delaytime %ld usec\n", sleep_usec_time, between_usec_time);
  }
  return;
}
...
void set_run_usec(long time) {
  ...
  PRINT_STATUS("Setting CS System: runtime %ld usec, delaytime %ld usec\n", sleep_usec_time, between_usec_time);
}
