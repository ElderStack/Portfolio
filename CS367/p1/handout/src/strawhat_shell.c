 ...
 * - strawhat_shell.c (StrawHat VM)
 * - Copyright: Prof. Kevin Andrea, George Mason University.  All Rights Reserved
 * - Date: Jan 2025
 *
 *   User Interaction Shell for the StrawHat VM Environment
 */
...
// Middle of #if FEATURE_SUSPEND == 1
...
static void run_kill(Process_data_s *data) {
  ...
  if(pid > 1) {
    ...
    if(succ == 0) {
      PRINT_STATUS("Process with PID %d has been killed.\n", pid);
    }
    else {
      PRINT_WARNING("No such process with PID %d\n", pid);
    }
  }
  else {
...
static void run_runtime(Process_data_s *data) {
  ...
  // Otherwise, provide the user some help.
    ...
    PRINT_INFO("The minimum runtime allowed is %ld usec\n", SLEEP_MIN_USEC);
    PRINT_INFO("The maximum runtime allowed is %ld usec\n", SLEEP_MAX_USEC);
    PRINT_INFO("The current runtime is %ld usec\n", get_run_usec());
    return;
  }
}
...
static void run_delaytime(Process_data_s *data) {
  ...
  // Otherwise, provide the user some help.
    ...
    PRINT_INFO("The minimum runtime allowed is %ld usec\n", BETWEEN_MIN_USEC);
    PRINT_INFO("The maximum runtime allowed is %ld usec\n", BETWEEN_MAX_USEC);
    PRINT_INFO("The current delaytime is %ld usec\n", get_between_usec());
    return;
  }
}
...
static int get_user_input(char *line, char *hline) {
  ...
  if(line[0] == 27) {
    ...
    if(line[0] == '\n') {
      ...
      PRINT_INFO("Re-executing prior command: %s\n", line);
    }
  }
...
static void print_process_data(Process_data_s *data) {
  ...
  PRINT_DEBUG( "| - [PID: %d]\n", data->pid);
  PRINT_DEBUG( "| - [CMD: %s]\n", data->cmd);
  PRINT_DEBUG( "| - [Is High-Pri: %s]\n", data->is_high?"Yes":"No");
  PRINT_DEBUG( "| - [Is Critical: %s]\n", data->is_critical?"Yes":"No");
  for(int i = 0; i < MAX_ARGS && data->argv[i] != NULL; i++) {
    PRINT_DEBUG( "| - [Arg %2d: %s]\n", i, data->argv[i]);
  }
  PRINT_DEBUG(".----------------------------\n");
...
static Process_data_s *parse_input(char *str) {
  ...
  // Optionally restrict commands to local directory binaries only (set in inc/strawhat_settings.h)
...
#if FEATURE_SUSPEND
  PRINT_STATUS( "| suspend X   Suspends the Running or Ready Process with PID X.\n");
  PRINT_STATUS( "| resume X    Resumes the Suspended Process with PID X.\n"); 
#endif  
