...
unsigned int fraction_to_binary(long fraction_part, int precision) {
   ...
   * - Repeats until the resulting binary value is the bit width
   */
  for(i = 0; i < BIT_WIDTH_RESULT; i++) {
    result <<= 1;
    fraction_part <<= 1;
...
long binary_to_fraction(unsigned int binary, int *precision) {
  ...
  *precision = 0;
  ...
  for(i = 5; binary; i*= 5, multiplier /= 10) {
    if(binary & (1<<(BIT_WIDTH_RESULT - 1))) {
      result += (i * multiplier);
    }
    binary <<= 1;
  }
...
void print_value(const char *msg, smallfp_s value) {
  printf("\r%s%s: %s", GREEN, msg, CYAN);
  print_binary_digits_precision(value, SMALLFP_TOTAL_BITS);
  printf(" (0x%03x)%s%c", value & ((1 << SMALLFP_TOTAL_BITS)-1), RST, IS_INTERACTIVE?'\n':'\0');
}
...
void print_binary_fraction(unsigned int val) {
  if(val) {
    while(val) {
      printf("%d", (val >> (BIT_WIDTH_RESULT-1))&1);
      val <<= 1;
    }
  }
  else {
...
static void print_binary_digits_precision(unsigned int val, int precision) {
  if(precision == 0) {
    ...
    print_binary_digits_precision((val>>1)&(~(1<<(BIT_WIDTH_RESULT-1))), precision - 1);
    printf("%d", val&0x1);
    if(precision == SMALLFP_SIGN_BITS) printf(" ");
    else if(precision == SMALLFP_SIGN_BITS + SMALLFP_EXP_BITS) printf(" ");
  }
} 
...
static void print_binary_digits(unsigned int val) {
  if(val == 0) {
    ...
    print_binary_digits((val>>1)&(~(1<<(BIT_WIDTH_RESULT-1))));
    printf("%d", val&0x1);
  }
} 
